...
- 文件：`src/proxy/proxy-http-handler.js`
- 关键方法：
  - `downloader.ensureCached(downloadUrlObj, cachePath, downloadRequestHeaders)`
...
Gradle 无论走 HTTP 还是 HTTPS 下载库，最终都会汇聚到本项目统一的「缓存路径计算 -> 缓存命中判断 -> 回源下载（单/多线程）-> `.temp` 原子落盘 -> 返回文件」主链路；HTTPS 仅多了 CONNECT 与可选 MITM 解密这层入口处理。